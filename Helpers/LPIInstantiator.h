		...
		public:
			...
			*/
			LPI* InstantiateLPI() {
				...
				switch (opcode) {
					case 6:
						this->lpi = new BlocksNonAnimatedLPI(ledConfig, stringProcessor);
						break;
				}

				// Ensure the LPI is initialised fully by resetting the intenral state
				this->lpi->Reset(this->lpiInstruction);

				return this->lpi;
			}

			/*!
			  @brief   Frees memory allocated to objects that were required for testing.
			*/
			void CleanUp() {
			...
			/*!
			  ...
			  @param   expectedRi	Pointer to the string that specifis the RI value that is expected.
			  @param   riBuffer		Pointer to the FixedSizeCharBuffer that contains the actual RI value.
			  @returns bool			True if the RI buffer contains the expected value.
			*/
